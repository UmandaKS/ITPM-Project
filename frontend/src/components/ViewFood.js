...
function ViewFood() {
  ...
  const { addToast } = useToasts(); // This is the toast notification function for food remaining weight
  ...
  function getFoods() {
    axios
      ...
      .then((res) => {
        ...
        // Here, we check if any food item has remainingWeight below 50
        res.data.forEach(food => {
          if (food.remainingWeight < 50) {
            addToast(`${food.name} is below 50 kg`, { appearance: 'warning', autoDismiss: true });
          }
        });
      })
      .catch((err) => {
